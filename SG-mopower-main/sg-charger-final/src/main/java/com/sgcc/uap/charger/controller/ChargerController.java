...
public class ChargerController {
	 ...
	 * @return
	 */
	@RequestMapping(value = "/chaxun/{Stationid}/{status}")
	public WrappedResult queryByStationidAndStatus(@PathVariable String Stationid,@PathVariable String status) {
		try {
			QueryResultObject result = new QueryResultObject();
			result = chargerService.queryChargerByStationidAndChargerstatus(Stationid,status);
			logger.info("更新成功");
			return WrappedResult.successWrapedResult(result);
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
	...
	public WrappedResult updateStatusById( @PathVariable String id ,@PathVariable String status) {
		try {
			QueryResultObject result = new QueryResultObject();
			result.setFormItems(chargerService.saveChargerStatusById(id, status));
			logger.info("更新成功"); 
			return WrappedResult.successWrapedResult(result);
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
